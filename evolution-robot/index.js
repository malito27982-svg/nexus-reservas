...
SETOR: ONDE FICA / COMO É (Lucas 30/08 — você disse pra uma cliente que "o Aquário fica coladinho na área kids", e isso NÃO está escrito em lugar nenhum; você inventou, e a casa teve que desmentir): ao oferecer um setor você pode dizer o NOME dele e se ele COMPORTA o grupo — mais nada. PROIBIDO TERMINANTEMENTE descrever onde o setor fica, o que tem perto, se é coberto, aberto, calmo, arejado, com vista, perto do palco/banheiro/kids/estacionamento, ou qualquer característica que não esteja escrita, palavra por palavra, nas INFORMAÇÕES DA CASA. Parece um detalhe simpático e é o jeito mais fácil de mentir pro cliente: ele escolhe o setor por causa disso e chega na casa achando outra coisa. Se perguntarem como é ou onde fica um setor e não estiver cadastrado, diga que quem descreve certinho é um atendente e chame chamar_atendente.
SETOR: ALOCAÇÃO AUTOMÁTICA (Lucas 02/09 — muitos clientes não conhecem a casa/as áreas e ficam confusos quando perguntados): NÃO pergunte ao cliente qual setor ele quer. Depois de consultar_disponibilidade, escolha VOCÊ MESMO o primeiro setor da lista com vaga suficiente pro grupo, e apenas informe qual foi escolhido (ex.: "Vou reservar no Deck para vocês"). SÓ use um setor diferente se o próprio cliente, por conta própria, disser que já conhece a casa e prefere um setor específico — aí sim respeite o pedido dele, se houver vaga.
AGENDA CHEIA / SEM SETOR COM VAGA (Lucas 02/09): se consultar_disponibilidade não trouxer NENHUM setor com vaga suficiente pro grupo na data pedida, NÃO diga simplesmente "não temos disponibilidade" e pare por aí — chame chamar_atendente explicando que é dia de agenda cheia, pra a casa verificar se dá uma exceção/encaixe manual.
...
FESTAS DE FIM DE ANO (Lucas 27/08 — dia 26 e 27/08 dois clientes pediram confraternização de 55 e de 100 pessoas e o robô não soube responder nada): temos PACOTE FECHADO de fim de ano, open food + open bar, um valor por pessoa. Assim que o cliente falar em confraternização, festa da empresa, amigo secreto, ceia, Natal, fim de ano, evento fechado ou grupo grande em novembro/dezembro, chame enviar_pacote_fim_de_ano — o PDF completo chega no chat — e explique os três planos com os números REAIS abaixo. NUNCA invente valor, item ou condição que não esteja aqui.
  • Vale para eventos de 20/11/2026 a 31/12/2026, a partir de 10 pessoas, com 3 horas de duração. Taxa de serviço NÃO inclusa (cobrada à parte no fechamento).
  ...
  • Petiscos e bebidas liberados SÓ durante as 3 horas contratadas; hora adicional é cobrada à parte. O pacote é só para evento reservado e não substitui o cardápio da casa. Outras marcas, doses e garrafas seguem o cardápio, à parte.
...
async function gerarFlyerGemini(selfie, ctx, ocasiao, extra) {
  ...
  const prompt = `Crie um FLYER VERTICAL 9:16 de RESERVA CONFIRMADA do "${ctx?.casa || 'Botequim São Paulo'}".${fundo ? ' Use a ' + (selfie ? 'PRIMEIRA ' : '') + 'imagem (ambiente REAL do bar) como CENÁRIO/FUNDO.' : ''}${selfie ? ' A PESSOA da ' + (fundo ? 'SEGUNDA ' : '') + 'foto é o ELEMENTO PRINCIPAL: mostre o ROSTO GRANDE, em DESTAQUE e FIEL (rosto real dela).' : ''}${ocasiao ? ' CLIMA/TEMA da decoração e das cores (NÃO é pra escrever isso na imagem, é só inspiração visual): ' + ocasiao + '.' : ''} Estilo moderno, cores quentes, boteco premium noturno. Tipografia legível com EXATAMENTE estes textos, E NENHUM OUTRO: "RESERVA CONFIRMADA", nome "${ctx?.nome || ''}", data "${ctx?.data || ''}${hora ? ' às ' + hora : ''}" e setor "${ctx?.setor || ''}". PROIBIDO TERMINANTEMENTE: não escreva, não desenhe e não sugira NENHUMA promoção, cortesia, desconto, brinde, prêmio, item de cardápio, preço ou condição comercial (ex: "cortesia de aniversário", "champanhe grátis", "mínimo de X pessoas") — o bar NÃO autorizou nenhuma oferta neste flyer, é só a confirmação da reserva. Qualquer texto que não seja um dos 4 campos acima é uma promessa falsa pro cliente e é proibido.${extra ? ' AJUSTE: ' + extra + '.' : ''}`
  const parts = []
...
const pedidosPareamento = {} // instance -> ts do pedido feito no painel do tablet (27/08)
...
app.post('/painel/qr', async (req, res) => {
  ...
  // 27/08: o chip agora fica PARADO até alguém pedir o QR (antes martelava sozinho
  // e o QR só existia 6% do tempo). Quem está na casa usa ESTE painel, não o do
  // escritório — então abrir a tela do QR aqui é o pedido: registra e o rv-agent
  // pega no próximo carteiro (3,5s) e abre a janela de 5 min de QR contínuo.
  pedidosPareamento[inst] = Date.now()
  res.json({ ok: false, aguarde: true, erro: 'Ligando o WhatsApp desta unidade… o QR aparece em alguns segundos. Deixe esta tela aberta.' })
})
...
// 27/08: canal de pedido de pareamento. A nuvem não alcança o PC do escritório,
// então o rv-agent é quem pergunta — no mesmo ciclo do carteiro. Um pedido vale
// 60s: se o agente estiver fora do ar na hora, o pedido morre e a pessoa toca de
// novo, em vez de abrir uma janela de QR sozinha meia hora depois.
app.get('/rv/parear-pedidos', (req, res) => {
  if (!authAgent(req, res)) return
  const agora = Date.now()
  const insts = Object.keys(pedidosPareamento).filter((i) => agora - pedidosPareamento[i] < 60000)
  for (const i of Object.keys(pedidosPareamento)) delete pedidosPareamento[i]
  res.json({ insts })
})
...
async function processarMensagem(body) {
  {
    ...
    if ((conv.flyer_etapa || conv.flyer_feedback) && /reserva|card[aá]pio|\bmenu\b|delivery|hor[aá]rio|endere[çc]o|d[uú]vida|cancelar|reclama|cortesia|desconto|promoç|brinde|gr[aá]tis|presente/i.test(texto) && !/n[ãa]o quero|s[óo] o bar|s[óo] a arte|minha foto|selfie|com foto/i.test(texto)) {
